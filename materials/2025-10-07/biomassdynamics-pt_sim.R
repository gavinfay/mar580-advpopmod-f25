...
prod_model_fun(floundah_data, parameters)
...
## Make a function object
obj <- MakeADFun(cmb(prod_model_fun, floundah_data), parameters, 
                 random = c("log_b"),
                 map = list(log_m_est=factor(NA), log_sigma_c = factor(NA),
                 #map = list(log_sigma_c = factor(NA),
                            log_sigma_proc = factor(NA))) #,
...
simulated_data <- map(1:2,function(x){
  sim <- obj$simulate();
  sim$log_b <- NULL
  return(sim)})
...
par_store <- NULL
for (isim in 1:2) {
obj <- MakeADFun(cmb(prod_model_fun, simulated_data[[isim]]), parameters, 
                 random = c("log_b"),
                 #map = list(log_m_est=factor(NA), log_sigma_c = factor(NA),
                 map = list(log_sigma_c = factor(NA),
                            log_sigma_proc = factor(NA))) #,
